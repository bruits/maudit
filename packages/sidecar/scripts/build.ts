...
async function buildForPlatform(triple: string, outfile: string) {
	...
	for (let i = 0; i < 5; ++i) {
		try {
			let cmd = $`bun build --compile --minify --target=${triple} ./src/index.ts --outfile=${outfile} --env inline`;
...
async function build(triple: string) {
	let start = process.hrtime.bigint();

	let outfile = path.resolve(
		__dirname,
		`../../binaries/${triple.replace("bun-", "")}/maudit-sidecar${
			triple.startsWith("bun-windows") ? ".exe" : ""
		}`
	);
...
let sums = results.map(({ triple, sum }) => `${sum}  ${triple}`);
